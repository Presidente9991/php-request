...
if (!empty($blockedUntil) || $blocked) {
    ...
    $currentTimestamp = strtotime(date('Y-m-d H:i:s'));
    $blockedUntilTimestamp = strtotime($blockedUntil);
    if ($blockedUntilTimestamp > $currentTimestamp || $blocked) {
        // Если дата будущая или чекбокс активирован, сохраняем её в столбце blocked_until до достижения текущей даты сервера
        $blocked = true; // Устанавливаем статус блокировки
    } else {
        // Если дата уже прошла и чекбокс не активирован, оставляем текущее значение блокировки без изменений
        $blocked = false;
        $blockedUntil = null;
    }
} else {
    // Если дата не указана и чекбокс не активирован, сбрасываем срок блокировки
    $blockedUntil = null;
}
