...
checkAndUnblockUsers(); // Вызов функции для проверки и разблокировки пользователей
...
if (isset($_GET['users_id']) && is_numeric($_GET['users_id'])) {
    ...
    $_SESSION['current_users_id'] = $userId; // Сохраняем ID пользователя в сессии
    ...
    // Проверяем успешность запроса данных о пользователе
    if (!$result) {
    ...
    $unlimited_password_expiry = $user['unlimited_password_expiry'] === 't'; // Проверяем, является ли текущий пароль бессрочным
    $blocked = $user['blocked'] === 't'; // Проверяем, заблокирован ли пользователь
    $blocked_until = $user['blocked_until'] ? date('Y-m-d\TH:i', strtotime($user['blocked_until'])) : ''; // Преобразуем дату блокировки в нужный формат
} else {
    // Если передан некорректный идентификатор пользователя, устанавливаем ошибку и перенаправляем на страницу администрирования
...
?>
    ...
    <header class="header-section center">
        <?php
        // Выводим сообщения об успешном или неудачном обновлении данных пользователя
